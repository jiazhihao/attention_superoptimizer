...
__global__ void
  ...
  int const NUM_INPUT_LOADERS =
      aso::threadblock::KernelParams::MAX_NUM_DMEM_INPUTS;

  cutlass::MatrixCoord input_matrix_offset_base[NUM_INPUT_LOADERS];
  cutlass::MatrixCoord input_matrix_offset_stride[NUM_INPUT_LOADERS];
  int input_global_offset_base[NUM_INPUT_LOADERS];
  int input_global_offset_stride[NUM_INPUT_LOADERS];
  // Preprocessing input offsets
  for (int idx = 0; idx < params.num_dmem_inputs; idx++) {
    // The first num_dmem_inputs operators must all be InputLoaders
    if (params.operator_types[idx] != aso::type::TB_INPUT_OP) {
      assert(false && "The first num_dmem_inputs must all be input loaders");
    }
    int3 input_map = params.input_map[idx];
    int forloop_dim = params.forloop_dim[idx];
    aso::kernel::DTensor dtensor = params.dmem_inputs[idx];
    aso::threadblock::STensor stensor = params.smem_outputs[idx];
    ...
    input_matrix_offset_base[idx] = {tb_offset_row, tb_offset_column};
    int tb_offset_row_stride = 0, tb_offset_column_stride = 0;
    if (forloop_dim == num_dims - 2) {
      tb_offset_row_stride = stensor.dim[num_dims - 2];
    }
    if (forloop_dim == num_dims - 1) {
      tb_offset_column_stride = stensor.dim[num_dims - 1];
    }
    input_matrix_offset_stride[idx] = {tb_offset_row_stride,
                                       tb_offset_column_stride};
    ...
    int global_offset = 0, global_offset_stride = 0;
    if (num_dims > 2) {
      ...
      if (num_dims == 4) {
        strides[num_dims - 4] =
            strides[num_dims - 3] * dtensor.dim[num_dims - 3];
      }
      if (num_dims > 4) {
        assert(false && "Currently support up to 4D stensors");
      }
      ...
      if (forloop_dim < num_dims - 2 && forloop_dim >= 0) {
        global_offset_stride = stensor.dim[forloop_dim] * strides[forloop_dim];
      }
    }
    input_global_offset_base[idx] = global_offset;
    input_global_offset_stride[idx] = global_offset_stride;
  }
  for (int i = 0; i < params.forloop_range; i++) {
    ...
    for (int op = 0; op < params.num_operators; op++) {
      ...
      if (op_type == aso::type::TB_INPUT_OP) {
        ...
        int tb_offset_row = input_matrix_offset_base[op].row() +
                            i * input_matrix_offset_stride[op].row();
        int tb_offset_column = input_matrix_offset_base[op].column() +
                               i * input_matrix_offset_stride[op].column();
        int global_offset =
            input_global_offset_base[op] + i * input_global_offset_stride[op];
...
__global__ void
  ...
  for (int idx = 0; idx < params.num_dmem_inputs; idx++) {
    // The first num_dmem_inputs operators must all be InputLoaders
    assert(params.operator_types[idx] == aso::type::TB_INPUT_OP);
    int3 input_map = params.input_map[idx];
    int forloop_dim = params.forloop_dim[idx];
    aso::kernel::DTensor dtensor = params.dmem_inputs[idx];
    aso::threadblock::STensor stensor = params.smem_outputs[idx];
