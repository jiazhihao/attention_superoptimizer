...
__global__ void launch_matmul_kernel(
  STensor A, cutlass::half_t *A_data,
  STensor B, cutlass::half_t *B_data,
  STensor C, cutlass::half_t *C_data) {
  ...
  GenericMatmulExecutor executor(
    smem_buffer,
    A, B, C,
    threadIdx.x,
...
TEST(threadblock_tests, matmul) {
  ...
  cutlass::HostTensor<cutlass::half_t, cutlass::layout::RowMajor>
    ...
    C_ours(cutlass::MatrixCoord(m, n)),
    C_ref(cutlass::MatrixCoord(m, n));
  cutlass::HostTensor<cutlass::half_t, cutlass::layout::ColumnMajor>
    B(cutlass::MatrixCoord(k, n));
  ...
  STensor
    A_s = allocate_stensor(bgraph, A),
    B_s = allocate_stensor(bgraph, B),
  ...
  if (!cutlass::reference::host::TensorEquals(
    ...
    std::cerr << "Error - Our kernel differs from reference. Wrote computed and reference results to '" << filename << "'" << std::endl;
