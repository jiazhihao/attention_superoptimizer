...
class RowMajorOutputAccumulator {
public:
...
public:
  CUTLASS_DEVICE
  RowMajorOutputAccumulator(ElementType *dmem_ptr,
...
class GenericOutputAccumulator {
public:
  CUTLASS_DEVICE
  GenericOutputAccumulator(char *smem_buffer,
    ...
    if (stensor.layout == STensor::ROW_MAJOR) {
      if (stensor.dim[0] == 64 && stensor.dim[1] == 64) {
        ...
        using OutputAccumulator =
            RowMajorOutputAccumulator<cutlass::half_t, kRow, kColumn, kThreads>;
        OutputAccumulator saver(
